...
if form.lower() == 'n':
    ...
    # === CONFIG ===
    input_file = r"C:\Users\School\Documents\GitHub\CrowdWalk\crowdwalk\HigashiKU\log\individual\log_individual_pedestrians.csv" 
    ...
    grouped_df = pd.concat(grouped_results_clean, ignore_index=True) # Use clean list
    totalV_df = pd.concat(totalV_results_clean, ignore_index=True)   # Use clean list
    ...
    combined.to_csv(r"C:\Users\School\Documents\GitHub\CrowdWalk\crowdwalk\HigashiKU\logoutput\velocity_pivot.csv")
    print("Combined velocity and agent count data saved to velocity_pivot.csv")

elif form.lower() == 'y':
    # Load pre-processed data
    combined = pd.read_csv(r"C:\Users\School\Documents\GitHub\CrowdWalk\crowdwalk\HigashiKU\logoutput\velocity_pivot.csv")
    
    # Separate back into velocity and agent count pivots
    ...
    # Also load grouped_df for visualizations
    grouped_df = pd.read_csv(r"C:\Users\School\Documents\GitHub\CrowdWalk\crowdwalk\HigashiKU\logoutput\aggregated_pedestrians.csv")
    df_viz = grouped_df.sample(n=min(100000, len(grouped_df)), random_state=42)  # Sample for visualization
...
plt.title('各道路の毎秒ごとの人口密度 (Top 10 Links)', fontname="Meiryo")
plt.xlabel('時間[秒]', fontname="Meiryo") # Error line now fixed!
plt.ylabel('エージェント数', fontname="Meiryo")
plt.legend(title='Link ID', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig(r"C:\Users\School\Documents\GitHub\CrowdWalk\crowdwalk\HigashiKU\logoutput\agent_count_top10_stacked_bar_"+dt+".png")
plt.close()
...
plt.title('各道路の毎秒ごとの合計速度 (Top 10 Links)', fontname="Meiryo")
plt.xlabel('時間[秒]', fontname="Meiryo")
plt.ylabel('速度 [m/s]', fontname="Meiryo")
plt.legend(title='Link ID', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.grid(True, linestyle='--', alpha=0.7) 
plt.tight_layout()
plt.savefig(r"C:\Users\School\Documents\GitHub\CrowdWalk\crowdwalk\HigashiKU\logoutput\total_velocity_top10_line_"+dt+".png")
plt.close() 
...
#population/(length*width) = density per m^2